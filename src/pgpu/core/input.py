...
class InputAxis:
    ...
    def _update(self):
        any_press = False
        if (self.pos and Input.keys_pressed[self.pos]) or (
            self.alt_p and Input.keys_pressed[self.alt_p]
        ):
            self.value += self.elasticity * Time.delta_time
            if self.value > 1:
                self.value = 1
            any_press = True
        if (self.neg and Input.keys_pressed[self.neg]) or (
            self.alt_n and Input.keys_pressed[self.alt_n]
        ):
            self.value -= self.elasticity * Time.delta_time
            if self.value < -1:
                self.value = -1
...
class Input:
    ...
    def _event(cls, event):
        if event.type == pygame.KEYDOWN:
            ...
            cls._key_data[event.key] = 2
        elif event.type == pygame.MOUSEBUTTONDOWN:
            cls.any_mouse = True
            cls._mouse_data[event.button] = 1
        elif event.type == pygame.MOUSEBUTTONUP:
            cls._mouse_data[event.button] = 2
    ...
    def add_axis(
        cls,
        name: str,
        positive_key: int = None,
        negative_key: int = None,
        alt_positive: int = None,
        alt_negative: int = None,
        elasticity: float = None,
    ):
        if name in cls._axis:
            return
        if elasticity is None:
            elasticity = cls.default_elasticity
        cls._axis[name] = InputAxis(
            positive_key, negative_key, alt_positive, alt_negative, elasticity
        )
