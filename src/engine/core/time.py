...
class Timer:
    def __init__(self, cooldown:int, callback=None, start_active:bool=True):
        self.cooldown:int = cooldown
        self.callback = callback
        self.active:bool = False
        self._start_time:int = -1
        if start_active: self.activate()
...
class Time:
    ...
    @classmethod
    def invoke(cls, function, time_ms:int, name:str=None):
    ...
    def stop_invoke(cls, name):
        for invoke in list(cls._invokes):
            if invoke[-1] == name: cls._invokes.remove(invoke)
    
    @classmethod
    def add_timer(cls, name:str, cooldown:int, callback=None, start_active:bool=True, auto_update:bool=True)->Timer:
        cls._timers[name] = (new_timer := Timer(cooldown,callback))
        if start_active: new_timer.activate()
